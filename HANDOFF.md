...
v1.14（拆分 `lib/game.ts` 和 `app/page.tsx`，不改功能）已完成，见 `CHANGELOG.md`。正式和 staging 仍要各自执行 `npx wrangler secret put VAPID_PRIVATE_KEY`，值与 `work/vapid-private.txt` 相同，不要把私钥贴进聊天或仓库。接下来只做依赖自动更新。

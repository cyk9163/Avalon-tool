...
v1.12 到 v1.15 都已完成，见 `CHANGELOG.md`。没有下一项排期功能。

正式和 staging 仍要各自执行 `npx wrangler secret put VAPID_PRIVATE_KEY`，值与 `work/vapid-private.txt` 相同，不要把私钥贴进聊天或仓库。没配置时对局照常进行，只是不发锁屏通知。

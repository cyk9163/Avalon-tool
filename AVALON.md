...
- 第一阶段使用 Sites 支持的 Cloudflare D1 持久化和每 3 秒同步，后台页面暂停同步。规划中的 Durable Objects/WebSocket 尚未接入。
...
要求 Node 22.13+，推荐 Node 24。安装 `npm ci`，运行 `npm run dev`。

本机 npm 的命令脚本有路径解析问题时，直接运行 `node scripts/run-framework.mjs dev` 或 `node scripts/run-framework.mjs build`。

数据库变更使用 `npm run db:generate`，构建后按项目 README 的 Local D1 migrations 说明应用到本地数据库；生产迁移由 Sites 发布流程应用。
...
3. 依据实际用量选择 D1 事件同步或迁移到 Durable Objects + WebSocket；部署到个人 Cloudflare 免费账户时需单独配置账户资源。Sites 发布不等同于已经配置个人 Cloudflare 免费账户。

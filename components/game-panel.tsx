...
export function GamePanel({ room, busy, connected, error, act, onNewGame }: Props) {
  ...
  if (!game) {
    ...
    return <section className="game-panel game-start">
      <div className="game-start-icon"><Crown size={28} /></div>
      <div><span className="eyebrow">EVERYONE IS READY</span><h2>全员就绪，开始第一轮。</h2><p>首任队长是 {room.firstLeader} 号 · {playerName(room.firstLeader ?? 1)}。面对面讨论后，在这里选队和投票。</p></div>
      {me.id === room.hostId
  ...
  return <section className="game-panel" aria-label="当前对局">
    <div className="game-topline"><span className="eyebrow">THE QUEST · 圆桌议事</span><span className="game-score"><span>正义 <b>{goodWins}</b></span><span>邪恶 <b>{evilWins}</b></span></span></div>
    <ol className="quest-track" aria-label="五次任务进度">
      {[1, 2, 3, 4, 5].map(number => {
        ...
        return <li key={number} className={`${quest ? quest.success ? "success" : "failure" : ""} ${current ? "current" : ""}`} aria-current={current ? "step" : undefined}>
          <span>任务 {number}</span><strong>{quest ? quest.success ? "成功" : "失败" : current ? "进行中" : "待开始"}</strong>
          <small>{quest ? `${quest.failCount} 张失败` : number === 4 && room.capacity >= 7 ? "需 2 张失败" : "1 张失败即败"}</small>
        </li>;
      })}
    </ol>

    {room.phase !== "finished" && room.phase !== "assassination" && <div className="quest-context">
      <span><Crown size={16} />队长 <b>{game.leaderSeat} 号 · {playerName(game.leaderSeat)}</b></span>
      <span>本次需 <b>{game.teamSize} 人</b></span>
      <span className={game.rejections === 4 ? "danger-copy" : ""}>连续否决 <b>{game.rejections} / 5</b></span>
    </div>}

    {room.phase === "team" && <div className="game-action">
      <div className="game-action-heading"><span className="step-icon"><UsersIcon /></span><div><h2>{leader ? "轮到你提议队伍" : "等待队长提议队伍"}</h2><p>{game.rejections === 4 ? "这是最后一次组队机会。再次否决，邪恶阵营将直接获胜。" : `面对面讨论后，由队长选出 ${game.teamSize} 位执行任务的玩家。`}</p></div></div>
      {leader ? <>
    ...
    {room.phase === "vote" && <div className="game-action">
      <div className="game-action-heading"><span className="step-icon"><ThumbsUp size={24} /></span><div><h2>这支队伍，值得信任吗？</h2><p>全员提交后统一公开结果。需 {Math.floor(room.capacity / 2) + 1} 人赞成，平票不通过。</p></div></div>
      <div className="voter-progress" aria-label={`已有 ${game.votedSeats.length} 人表决`}>{room.players.map(player => <span key={player.id} className={game.votedSeats.includes(player.seat) ? "submitted" : ""}>{player.seat}{game.votedSeats.includes(player.seat) && <Check size={12} />}</span>)}</div>
    ...
    {room.phase === "quest" && <div className="game-action">
      <div className="game-action-heading"><span className="step-icon"><LockKeyhole size={24} /></span><div><h2>任务进行中，秘密投下你的牌。</h2><p>{game.failsRequired === 2 ? "本任务至少出现 2 张失败牌才会失败。" : "只要出现 1 张失败牌，本任务就会失败。"}个人任务票始终保密。</p></div></div>
      <p className="ballot-count" role="status"><b>{game.submittedQuestCount}</b> / {game.teamSize}<span>任务票已提交</span></p>
    ...
    {room.phase === "assassination" && <div className="game-action assassination">
      <div className="game-action-heading"><span className="step-icon"><Swords size={27} /></span><div><h2>三次任务成功，最后的暗影。</h2><p>邪恶阵营可以公开讨论。刺客只有一次机会：找出梅林，即可逆转结局。</p></div></div>
    ...
    {room.phase === "finished" && game.result && <div className={`game-result ${game.result.winner}`}>
      <Trophy size={38} strokeWidth={1.3} /><span className="eyebrow">THE STORY IS TOLD</span><h2>{game.result.winner === "good" ? "正义守住了圆桌。" : "暗影笼罩了圆桌。"}</h2><p>{reasonCopy[game.result.reason]}</p>
    ...
    <AlertDialog open={!!pending} onOpenChange={open => { if (!open && !busy) setPending(null); }}>
      <AlertDialogContent><AlertDialogTitle>{pending?.title}</AlertDialogTitle><AlertDialogDescription>{pending?.description}</AlertDialogDescription>{feedback && <p className="ballot-error" role="alert">{feedback}</p>}<AlertDialogFooter><AlertDialogCancel disabled={busy}>再想一下</AlertDialogCancel><AlertDialogAction disabled={blocked} onClick={event => { event.preventDefault(); void confirmPending(); }}>{busy ? "正在提交…" : pending?.label}</AlertDialogAction></AlertDialogFooter></AlertDialogContent>
    </AlertDialog>
    <Dialog open={ballotOpen && room.phase === "quest"} onOpenChange={open => { setBallotOpen(open); if (!open) setCard(null); }}>
      <DialogContent className="private-ballot-dialog"><DialogTitle><LockKeyhole size={20} />你的秘密任务票</DialogTitle><DialogDescription>先确认周围没有人在看屏幕。选好后点击提交，无法改票。</DialogDescription>
...
function UsersIcon() { return <Crown size={25} />; }

...
export function RecoveryCodeCard({ room }: { room: RoomView }) {
  ...
  return <section className="config-card recovery-card" aria-labelledby="recovery-card-title">
    <div className="config-heading"><h3 id="recovery-card-title"><KeyRound size={15} aria-hidden="true" />换设备恢复码</h3><span>仅你可见</span></div>
    {code ? <>
      <div className="recovery-code-row">
        ...
        <button type="button" className="icon-text-button" onClick={() => setVisible(value => !value)} aria-pressed={visible} onBlur={() => setVisible(false)}>
          {visible ? <EyeOff size={15} aria-hidden="true" /> : <Eye size={15} aria-hidden="true" />}{visible ? "隐藏" : "显示"}
        </button>
      ...
      <p className="action-note">手机没电或换浏览器时，在新设备打开本房间，选择「我本来就在这桌，换了设备」并输入此码即可回到座位。原设备会立即失效，恢复码用后自动更换。请勿给别人看。</p>
      {copyFailed && <p className="action-note" role="status">无法复制，请手动记下。</p>}
    </> : <p className="action-note">这个房间创建于旧版本，没有恢复码。需要换设备时，可以请房主批准接管你的座位。</p>}
  </section>;
}
...
export function SeatRecovery({ room, busy, connected, act }: { room: RoomView; busy: boolean; connected: boolean; act: Act }) {
  ...
  if (request) {
    return <div className="seat-recovery waiting" role="status">
      ...
      <div>
        <strong>已请求接管 {request.seat} 号座位</strong>
        <p className="takeover-code large">核对码 <b>{request.verifyCode}</b></p>
        <p>请把这个号码给房主看，房主核对一致后批准（发出请求 1 分钟后才能批准，原设备可在此期间拒绝）。批准后，这台设备会自动显示你的身份。</p>
        <button type="button" className="text-button subtle" disabled={blocked} onClick={() => void act("takeover-cancel")}><X size={15} aria-hidden="true" />撤回请求</button>
      </div>
    </div>;
  }
  ...
  if (!open) {
    return <button type="button" className="secondary-button wide seat-recovery-toggle" onClick={() => setOpen(true)}>
      <Smartphone size={16} aria-hidden="true" />我本来就在这桌，换了设备
    </button>;
  }
  ...
  return <form className="seat-recovery" onSubmit={event => { event.preventDefault(); void submit(); }}>
    <div className="seat-recovery-heading"><strong>回到原来的座位</strong><button type="button" className="icon-button small" aria-label="收起" onClick={() => setOpen(false)}><X size={16} /></button></div>
    <fieldset>
      <legend>你原来坐在几号？</legend>
    ...
    <div className="seat-recovery-methods" role="radiogroup" aria-label="恢复方式">
      <button type="button" role="radio" aria-checked={method === "code"} className={method === "code" ? "selected" : ""} onClick={() => setMethod("code")}><KeyRound size={15} aria-hidden="true" />输入恢复码</button>
      <button type="button" role="radio" aria-checked={method === "host"} className={method === "host" ? "selected" : ""} onClick={() => setMethod("host")}><UserCheck size={15} aria-hidden="true" />请房主批准</button>
    </div>
    {method === "code" ? <label className="field">恢复码
    ...
    </label> : <p className="action-note">{chosen !== null && chosen === hostSeat ? "房主的座位只能用恢复码恢复。" : "房主批准前请当面确认是你本人。批准后，原来的设备会立即失效。"}</p>}
    <button className="primary-button" disabled={blocked || !chosen || (method === "code" ? code.replace(/[\s-]/g, "").length !== 10 : chosen === hostSeat)}>
      {method === "code" ? "恢复座位" : "发送请求"}
    </button>
  </form>;
}

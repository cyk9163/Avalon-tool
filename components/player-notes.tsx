  ...
  return (
    <section id="room-notes" className="player-notes" aria-label={t("我的推理笔记")}>
      ...
      {editing && createPortal(
        <div className="notes-editor" role="dialog" aria-modal="true" aria-labelledby="notes-editor-title">
          <div className="notes-editor-bar">
            <h2 id="notes-editor-title">{t("我的发言准备")}</h2>
            <button type="button" className="primary-button" onClick={() => setEditing(false)}>{t("完成")}</button>
          </div>
          <p>{t("只保存在这台设备，不会发给服务器或其他玩家；同房再开后换一张新笔记。点圆桌上其他玩家的头像，选一个字标在右上角。")}</p>
          <textarea
            ref={draftRef}

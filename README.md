...
扩展角色限定 7 人及以上。兰斯洛特采用官方阵营转换规则（变体 2）：两人互不认识；开局公开 3 张忠诚牌（从 2 张「转换」、3 张「不变」中抽出），对应第 3、4、5 轮，翻到「转换」起两人互换阵营；属于正义时只能出成功，属于邪恶时只能出失败。疯子必须出失败牌；野蛮人在第四、第五次任务只能出成功牌；揭露者在第二次任务失败后公开；牧师私密得知第一任队长的阵营。自定义板子还可启用湖中仙女，在第 2、3、4 次任务后由当前持有者私密查验一位玩家的阵营，再把令牌交给对方；查验结果只返回给查验者。
...
- 右上角「EN / 中文」按钮切换语言，选择保存在 Cookie `avalon_lang`。没有选择时默认中文，不看浏览器语言。
...
手机浏览器测试：首次运行前执行 `npx playwright install webkit chromium`，然后 `npm run test:e2e`（会复用已启动的本地开发服务器，没有时自动启动）。测试用 WebKit 模拟 iPhone 13、Chromium 模拟 Pixel 7，覆盖圆桌选人、亮车／改车／发起表决、实时同步、表决、按住查看身份、断网重连和手机上无横向滚动。可用 `E2E_BASE_URL` 指定地址。
...
- `components/game-table.tsx`、`components/rules-card.tsx`：对局圆桌主视图（含队长亮车）与本局规则卡。

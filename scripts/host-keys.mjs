...
// generate accepts --count N (1-50, default 10); publish accepts --env staging
// to update the staging Worker's allowlist instead of production.
...
const count = countOption === null ? 10 : Number(countOption);
if (!['generate', 'publish'].includes(action) || !target || (wranglerEnv !== null && wranglerEnv !== 'staging')
  || !Number.isInteger(count) || count < 1 || count > 50 || (action === 'publish' && countOption !== null)) {
  console.error('Usage: node scripts/host-keys.mjs generate work/private-keys.txt [--count N]');
  console.error('       node scripts/host-keys.mjs publish work/private-keys.txt [--env staging]');
  process.exit(1);
}
...
if (action === 'generate') {
  ...
  console.log(`Generated ${count} private key${count === 1 ? '' : 's'}: ${file}`);
} else {

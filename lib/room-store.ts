...
import {GameError,type Room,mutateRoom,roomView,type Preset,rolePool,nickname,randomInt} from "./game";
...
export async function createRoom(key:string,input:Record<string,unknown>){
  ...
  for(let i=0;i<8;i++){
    ...
    const room:Room={code,round:1,capacity,preset,...(preset==="custom"?{customRoles:roles,ladyOfLake}:{}),phase:"lobby",hostId:id,hostRevision:0,players:[{id,key,name,seat:1,ready:false,confirmed:false}],createdAt:Date.now(),expiresAt:Date.now()+86400000,requestId};
...
export async function changeRoom(code:string,key:string,action:string,input:Record<string,unknown>){
  for(let attempt=0;attempt<24;attempt++){
    ...
    if(result.meta.changes===1)return roomView(room,key,version+1);

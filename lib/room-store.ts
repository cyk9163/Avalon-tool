...
export async function createRoom(key:string,input:Record<string,unknown>){
  ...
  rolePool(capacity,preset);const name=nickname(input.name);
  ...
  for(let i=0;i<8;i++){
    ...
    const room:Room={code,capacity,preset,phase:"lobby",hostId:id,players:[{id,key,name,seat:1,ready:false,confirmed:false}],createdAt:Date.now(),expiresAt:Date.now()+86400000,requestId};

...
import { careerAchievementIds, gameAchievementIds } from "./achievements.ts";
...
export async function recordAccountGames(room: Room): Promise<void> {
  ...
  for (const player of room.players) {
    ...
    const stored = await env.DB.prepare("SELECT role, side, winner, mvp FROM account_games WHERE account_id = ?").bind(player.accountId).all<{ role: string; side: string; winner: string; mvp: number }>();

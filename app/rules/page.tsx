    ...
    >
      ...
      <section id="flow">
        ...
        <ol className="doc-steps">
          ...
          <li><strong>{t("全员表决")}</strong>{t("所有人对这支队伍投赞成或反对。严格过半才通过，平票算否决。否决后队长交给下一位。同一任务连续两次否决后，第三车不表决，直接执行任务。")}</li>
      ...
      <section id="sight">
        ...
        <ul>
          ...
          <li>{rich(t("<b>邪恶玩家</b>互相认识，并且知道彼此的具体角色（包括邪恶兰斯洛特），但看不见奥伯伦；<b>奥伯伦</b>也不认识其他邪恶玩家。（本站规则，官方规则中坏人只知道彼此是坏人。）"))}</li>
          <li>{rich(t("<b>两位兰斯洛特</b>互不认识。开局时公开 3 张忠诚牌，分别对应第 3、4、5 轮；翻到「转换」的那一轮起，两人互换阵营。属于正义时只能出成功，属于邪恶时只能出失败；湖中仙女查到的是他当前的阵营，结局按最终阵营揭晓。"))}</li>
      ...
      <section id="win">
        ...
        <ul>
          <li><strong>{t("邪恶立即获胜")}</strong>{t("：三次任务失败。同一任务连续两次否决后，第三车直接执行任务。")}</li>
          <li><strong>{t("刺杀")}</strong>{t("：好人完成三次任务后，刺客选择一名他认为是梅林的玩家。选中梅林则邪恶获胜，否则正义获胜。刺杀只有一次，提交后不能更改。")}</li>
          <li><strong>{t("刺客随时出刀（本站规则）")}</strong>{t("：对局开始后，刺客可以在任意时刻刺杀一次；刺中梅林邪恶立即获胜，刺错正义立即获胜。如果一直没有出刀，好人三次任务成功后照常进入最后刺杀。（本站规则，官方规则中只能在三次任务成功后刺杀。）")}</li>

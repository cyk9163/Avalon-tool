...
export default function Home(){
  ...
  return <main className="app-shell"><header className="topbar"><Brand onHome={back}/><div className="header-right"><span className="edition">线下阿瓦隆助手</span><button className="icon-button" aria-label="使用说明" onClick={()=>setHelp(true)}><CircleHelp size={19}/></button></div></header>
    ...
    <section className="room-page"><div className="room-heading"><div><span className="eyebrow">{room.phase==="lobby"?"TAKE YOUR SEAT":room.game?"THE ROUND TABLE":"YOUR SECRET"}</span><h1>{room.phase==="lobby"?"围坐，等待朋友。":room.phase==="ready"?"身份就绪，开始推理。":room.phase==="finished"?"故事落幕，复盘这一局。":room.game?"每一票，都留下线索。":"请查看你的身份。"}</h1><p className="room-meta">房间 <strong>{room.code}</strong><span>·</span>{room.capacity} 人 · {PRESETS[room.preset].name}</p></div><button className="secondary-button" onClick={()=>setShare(true)}><QrCode size={18}/>邀请朋友</button></div>

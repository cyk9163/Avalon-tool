...
function RoleChips({roles}:{roles:Role[]}){
  const {t}=useI18n();
  const pool=roles;
  return <div className="role-chips">{[...new Set(pool)].map(role=><RoleInfoButton role={role} className={`role-chip ${ROLES[role].side}`} key={role}>{t(ROLES[role].name)}{pool.filter(r=>r===role).length>1?` ×${pool.filter(r=>r===role).length}`:""}</RoleInfoButton>)}</div>;
}

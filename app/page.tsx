...
function RoleChips({roles}:{roles:Role[]}){
  const {t}=useI18n();
  const pool=roles;
  return <div className="role-chips">{[...new Set(pool)].map(role=><span className={`role-chip ${ROLES[role].side}`} key={role}>{t(ROLES[role].name)}{pool.filter(r=>r===role).length>1?` ×${pool.filter(r=>r===role).length}`:""}</span>)}</div>;
}
...
export default function Home(){
  ...
  const [customSpecials,setCustomSpecials]=useState<Set<Role>>(()=>new Set(["merlin","assassin"])),[ladyOfLake,setLadyOfLake]=useState(false),[anytimeAssassin,setAnytimeAssassin]=useState(true);
  ...
  const create=async()=>{
    ...
    createId.current ||= crypto.randomUUID();const created=await act("create",{name,hostKey:hostKeyForSubmit(hostKey),capacity,preset,...(preset==="custom"?{roles:customRoles,ladyOfLake}:{}),anytimeAssassin,requestId:createId.current});if(created)setHostKey("");};
  ...
  return <main className={`app-shell ${room ? "is-room" : "is-home"}`}>
    ...
    {booting ? <section className="connection-splash" aria-live="polite"><span className="splash-emblem"><Crown size={32}/></span><h1>{t("圆桌正在就位")}</h1><p><LoaderCircle size={16} className="spin"/>{t("正在连接，恢复你的房间…")}</p></section> : !room ? <div className="home-layout">
        ...
        <Tabs value={mode} onValueChange={v=>{setMode(v as Mode);setError("");if(v==="join")setHostKey("");}} className="entry-tabs">
          ...
          <TabsContent value="create">
            <form className="form-stack" onSubmit={e=>{e.preventDefault();void create();}}>
              ...
              <label className="module-toggle rule-toggle"><input type="checkbox" checked={anytimeAssassin} onChange={e=>{setAnytimeAssassin(e.target.checked);createId.current="";}}/><span><strong>{t("刺客可随时出刀")}</strong><small>{t("对局中刺客可以在任意时刻刺杀一次：刺中梅林邪恶获胜，刺错正义获胜。关闭后按官方规则，只在三次任务成功后刺杀。")}</small></span></label>
      ...
      <section className="table-panel showcase" aria-label={t("当前角色配置预览")}>
        ...
        <div className="showcase-roster"><div className="alignment-line"><span><i/>{t("{n} 位好人",{n:capacity-EVIL_COUNTS[capacity]})}</span><span><i/>{t("{n} 位坏人",{n:EVIL_COUNTS[capacity]})}</span></div><RoleChips roles={previewRoles}/>{preset==="custom"&&ladyOfLake&&<p className="module-badge">{t("湖中仙女 · 已启用")}</p>}{anytimeAssassin&&<p className="module-badge">{t("刺客可随时出刀")}</p>}</div>
      ...
      <details className={`room-details ${room.game?"in-game":""}`} open={!room.game}>
        ...
        <div className="room-layout">
          ...
          <aside className="room-side">
            ...
            <section className="config-card"><div className="config-heading"><h3>{t("本局阵容")}</h3><span>{t(PRESETS[room.preset].name)}</span></div><div className="alignment-line"><span><i/>{t("{n} 位好人",{n:room.capacity-EVIL_COUNTS[room.capacity]})}</span><span><i/>{t("{n} 位坏人",{n:EVIL_COUNTS[room.capacity]})}</span></div><RoleChips roles={room.roles}/>{room.ladyOfLake&&<p className="module-badge">{t("湖中仙女 · 已启用")}</p>}{room.anytimeAssassin&&<p className="module-badge">{t("刺客可随时出刀")}</p>}<p className="action-note"><Shield size={13}/>{t("配置公开，个人身份私密分发。")}</p></section>

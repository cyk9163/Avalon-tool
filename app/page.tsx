...
export default function Home(){
  ...
  const [customSpecials,setCustomSpecials]=useState<Set<Role>>(()=>new Set(["merlin","assassin"])),[ladyOfLake,setLadyOfLake]=useState(false),[turnSpeech,setTurnSpeech]=useState(false);
  ...
  const create=async()=>{
    ...
    createId.current ||= crypto.randomUUID();const created=await act("create",{name,hostKey:hostKeyForSubmit(hostKey),capacity,preset,turnSpeech,...(preset==="custom"?{roles:customRoles,ladyOfLake}:{}),requestId:createId.current});if(created)setHostKey("");};

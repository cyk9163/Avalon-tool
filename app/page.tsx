...
export default function Home(){
  ...
  return <main className={`app-shell ${room ? "is-room" : "is-home"}${soloEmbed ? " is-solo-embed" : ""}`}>
    ...
    {booting ? <section className="connection-splash" aria-live="polite"><span className="splash-emblem"><Crown size={32}/></span><h1>{t("圆桌正在就位")}</h1><p><LoaderCircle size={16} className="spin"/>{t("正在连接，恢复你的房间…")}</p></section> : !room ? <div className="home-layout">
    ...
    </div> : <section className={`room-page${!room.game&&roomPanel==="table"?" is-lobby":""}${roomPanel==="notes"?" is-notes":""}${roomPanel==="identity"?" is-identity":""}`}>
      ...
      {!room.game&&<div className="room-layout">
          ...
          {!me&&<aside className="room-side"><section className="action-card"><span className="eyebrow">JOIN THE TABLE</span><h2>{room.phase==="lobby"?t("给自己留个座位。"):t("本局已经开始。")}</h2>{room.phase==="lobby"?<><label className="field">{t("你的昵称")}<input maxLength={12} autoComplete="nickname" placeholder={t("大家怎么称呼你")} value={name} onChange={e=>setName(e.target.value)}/></label><p className="muted-copy">{t("填好昵称后，在圆桌上选一个空位，就能加入朋友的房间。")}</p></>:<p className="muted-copy">{t("发身份后不能中途加入。如果你本来就在这桌，点左上角皇冠，用恢复码回到原来的座位。")}</p>}{room.namesHidden&&<p className="action-note"><LockKeyhole size={13}/>{t("通过房间码进入时不显示昵称；扫描房主的二维码可看到完整座位信息。")}</p>}<button className="text-button" onClick={back}><ArrowLeft size={16}/>{t("返回首页")}</button></section></aside>}
        </div>}
      </div>
      <div id="room-identity" hidden={roomPanel!=="identity"}>
        {me&&room.phase!=="lobby"?<section className="action-card identity-card"><div className="identity-heading"><span className="eyebrow"><Shield size={14}/>PRIVATE · {t("仅你可见")}</span><span className="identity-seat">{t("{n} 号",{n:me.seat})}</span></div><button className="reveal-button" disabled={!identity} onPointerDown={e=>{e.preventDefault();e.currentTarget.setPointerCapture(e.pointerId);setReveal(true);setSeen(true);}} onPointerUp={()=>setReveal(false)} onPointerCancel={()=>setReveal(false)} onLostPointerCapture={()=>setReveal(false)} onKeyDown={e=>{if(e.key===" "||e.key==="Enter"){e.preventDefault();setReveal(true);setSeen(true);}}} onKeyUp={e=>{if(e.key===" "||e.key==="Enter")setReveal(false);}} onBlur={()=>setReveal(false)} onContextMenu={e=>e.preventDefault()}>{reveal?<EyeOff size={18}/>:<Eye size={18}/>}{t("按住查看，松开隐藏")}</button><div className={`identity-surface ${reveal?"revealed":""}`} aria-live="off" onPointerDown={e=>{if(!identity)return;e.preventDefault();e.currentTarget.setPointerCapture(e.pointerId);setReveal(true);setSeen(true);}} onPointerUp={()=>setReveal(false)} onPointerCancel={()=>setReveal(false)} onLostPointerCapture={()=>setReveal(false)} onContextMenu={e=>e.preventDefault()}>{reveal&&identity?<><span className={`side-label ${identity.side}`}>{identity.side==="good"?t("正义阵营"):t("邪恶阵营")}</span><h3>{t(ROLES[identity.role].name)}</h3><p>{t(ROLES[identity.role].description)}</p><div className="identity-clues"><strong>{t("你知道的线索")}</strong>{identity.known.map(p=><div className="clue" key={p.seat}><span>{t("{n} 号",{n:p.seat})}</span><b>{p.name}</b><small>{ts(p.label)}</small></div>)}<p>{ts(identity.note)}</p></div></>:<div className="sealed"><span className="sealed-mark"><LockKeyhole size={36} strokeWidth={1.25}/></span><strong>{t("你的身份已密封")}</strong><p>{t("秘密只有你知道。")}<br/>{t("查看前，留意身边的目光。")}</p><span className="sealed-rule"/></div>}</div>{!room.game&&<button className={me.confirmed?"secondary-button wide ready-button":"primary-button"} disabled={busy||me.confirmed||!seen||!connected} onClick={()=>{setReveal(false);void act("confirm").then(done=>{if(done)setRoomPanel("table");});}}><Check size={18}/>{me.confirmed?t("我已确认身份"):t("我记住了，确认身份")}</button>}<p className="action-note">{room.game?t("切到后台时，身份会自动隐藏。"):room.phase==="ready"?t("全员已确认，等待房主开始对局。"):t("还有 {n} 人等待确认身份",{n:room.capacity-confirmedCount})}</p>{room.phase==="ready"&&<div className="ready-notice"><Crown size={19}/><div><strong>{t("第一任队长 · {n} 号",{n:room.firstLeader??""})}</strong><p>{t("房主点击上方「开始对局」，进入第一轮。")}</p></div></div>}</section>

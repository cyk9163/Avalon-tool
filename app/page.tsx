        ...
        <Tabs value={mode} onValueChange={v=>{setMode(v as Mode);setError("");if(v==="join")setHostKey("");}} className="entry-tabs">
          ...
          <TabsContent value="create">
            <form className="form-stack" onSubmit={e=>{e.preventDefault();void create();}}>
              ...
              {(preset==="full"||(preset==="custom"&&customSpecials.has("oberon")))&&<label className="module-toggle"><input type="checkbox" checked={knowsOberon} onChange={e=>{setKnowsOberon(e.target.checked);createId.current="";}}/><span><strong>{t("坏人认识奥伯伦")}</strong><small>{t("坏人知道奥伯伦是谁，奥伯伦不知道队友。")}</small></span></label>}
